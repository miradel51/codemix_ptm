...
def make_input(input_sentence, tokenizer, max_length):
    '''
...
class SimALMDataset(Dataset):
    def __init__(self, input_filenames, tokenizer, max_length):
        '''
        ...
        for i in range(n_examples):
            ...
            input_ids_query, attention_mask_query = make_input(inputs['query'][i], tokenizer, max_length)
            input_ids_label, attention_mask_label = make_input(inputs['label'][i], tokenizer, max_length)
...
class DataCollatorForSimALM:
    ...
    def __call__(self, examples):
        ...
        # 先将query和label的input_ids、attention_mask都转换成padded tensor的形式
        batch = {
        ...
        # 再对query和label的句子随机进行掩码
        batch['input_ids_query'], batch['mlm_labels_query'] = self.mask_tokens(batch['input_ids_query'])
        batch['input_ids_label'], batch['mlm_labels_label'] = self.mask_tokens(batch['input_ids_label'])
        return batch
    ...
    def mask_tokens(self, inputs):
        ...
        # 被掩码的单词有80%的概率替换成[MASK]
        indices_replaced = torch.bernoulli(torch.full(labels.shape, 0.8)).bool() & masked_indices
        inputs[indices_replaced] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)

        # 被掩码的单词有10%的概率被替换成随机单词
